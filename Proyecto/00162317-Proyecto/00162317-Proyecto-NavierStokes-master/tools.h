...
void obtenerDatos(istream &file,int nlines,int n,int mode,item* item_list){
    ...
    for(int i=0;i<n;i++){
        ...
        case INT_FLOAT:
            int e0; float r0;
            file >> e0 >> r0;
            item_list[i].setValues(0,0,0,0,e0,0,0,0, r0);
            break;
        ...
        case INT_INT_INT_INT_INT:
            int e1,e2,e3,e4,e5;
            file >> e1 >> e2 >> e3 >> e4 >>e5;
            item_list[i].setValues(e1,0,0,0,e2,e3,e4,e5,0);
            break;
        }
    }
}
...
void leerMallayCondiciones(mesh &m,char *filename){
    ...
    ifstream file;
    //Variables a ocupar
    float u_bar =0, nu = 0,rho = 0,f_x = 0,f_y = 0,f_z = 0;
    ...
    //Extension tipo dat que utiliza el GID
